import streamlit as st
import pandas as pd
import re
from datetime import datetime, timedelta
import os
import streamlit.components.v1 as components

# Configuração da página
st.set_page_config(page_title="RH Comunica", layout="centered", initial_sidebar_state="collapsed")

# --- ARQUIVOS DE DADOS ---
FILE_MENSAGENS = "MENSAGENS_ENVIADAS.csv"
FILE_COLABS = "DADOS_COLABORADORES.xlsx"

# Colunas que o sistema precisa ter obrigatoriamente
COLUNAS_MSG = ["id", "data_envio", "cpf_destino", "nome_destino", "mensagem", "lido", "data_leitura"]

# Inicializa ou corrige o arquivo de mensagens
def inicializar_mensagens():
    if not os.path.exists(FILE_MENSAGENS):
        pd.DataFrame(columns=COLUNAS_MSG).to_csv(FILE_MENSAGENS, index=False)
    else:
        # Se o arquivo existe, verifica se faltam colunas (como o 'id')
        df = pd.read_csv(FILE_MENSAGENS, dtype=str)
        mudou = False
        for col in COLUNAS_MSG:
            if col not in df.columns:
                df[col] = "" if col != "id" else [datetime.now().strftime("%Y%m%d%H%M%S") + str(i) for i in range(len(df))]
                mudou = True
        if mudou:
            df.to_csv(FILE_MENSAGENS, index=False)

inicializar_mensagens()

# --- FUNÇÕES DE APOIO ---
def limpar_cpf(cpf):
    s = str(cpf).split('.')[0]
    s = re.sub(r'\D', '', s)
    return s.zfill(11)

@st.cache_data(ttl=60)
def carregar_colaboradores():
    if os.path.exists(FILE_COLABS):
        try:
            df = pd.read_excel(FILE_COLABS)
            df.columns = [c.strip() for c in df.columns]
            df = df.dropna(subset=['CPF'])
            df['CPF_LIMPO'] = df['CPF'].apply(limpar_cpf)
            return df
        except: return None
    return None

def carregar_mensagens():
    df = pd.read_csv(FILE_MENSAGENS, dtype=str).fillna("")
    if not df.empty:
        # Tenta converter data para filtro de 48h
        df['dt_obj'] = pd.to_datetime(df['data_envio'], format="%d/%m/%Y %H:%M", errors='coerce')
    else:
        df['dt_obj'] = None
    return df

# --- COMPONENTE DE SOM E PERSISTÊNCIA (JavaScript) ---
def injetar_javascript():
    components.html("""
    <script>
    // Tocar som de notificação
    window.parent.tocarAlerta = function() {
        var audio = new Audio('https://notificationsounds.com/storage/sounds/file-sounds-1150-pristine.mp3');
        audio.play();
    }

    // Persistência de Login (Auto-login)
    const salvo = localStorage.getItem('rh_cpf_logado');
    if (salvo && !window.parent.location.href.includes('sair')) {
        // Envia o CPF salvo de volta para o Streamlit
        window.parent.postMessage({type: 'auto_login', cpf: salvo}, "*");
    }
    </script>
    """, height=0)

injetar_javascript()

# --- LÓGICA DE LOGIN ---
if "logado" not in st.session_state:
    st.session_state.update({"logado": False, "user_cpf": "", "is_admin": False, "user_nome": ""})

# Interface de Login
if not st.session_state.logado:
    st.title("📲 RH Comunica - Login")
    
    cpf_input = st.text_input("CPF (apenas números):", key="campo_cpf")
    
    col1, col2 = st.columns(2)
    with col1:
        if st.button("Entrar", use_container_width=True):
            if cpf_input == "000": # Senha Admin
                st.session_state.update({"logado": True, "is_admin": True})
                st.rerun()
                
            cpf_busca = limpar_cpf(cpf_input)
            if df_base is not None:
                user = df_base[df_base['CPF_LIMPO'] == cpf_busca]
                if not user.empty:
                    st.session_state.update({
                        "logado": True, "user_cpf": cpf_busca, 
                        "user_nome": str(user.iloc[0]['Nome'])
                    })
                    # Salva no navegador
                    components.html(f"<script>localStorage.setItem('rh_cpf_logado', '{cpf_busca}');</script>", height=0)
                    st.rerun()
                else: st.error("CPF não cadastrado.")
    
# --- DASHBOARD LOGADO ---
else:
    # Barra lateral com botão Sair que limpa a memória do navegador
    if st.sidebar.button("Logoff / Sair"):
        components.html("<script>localStorage.removeItem('rh_cpf_logado'); window.parent.location.reload();</script>", height=0)
        st.session_state.logado = False
        st.rerun()

    if st.session_state.is_admin:
        st.title("🛠️ Gestão RH")
        t1, t2, t3 = st.tabs(["🚀 Enviar", "📊 Relatório / Excluir", "⚙️ Base"])
        
        with t1:
            modo = st.radio("Destinatários:", ["Por Obra", "Lista Manual"], horizontal=True)
            cpfs_alvo = []
            if df_base is not None:
                if modo == "Por Obra":
                    obras = st.multiselect("Obras:", sorted([str(x) for x in df_base['OBRA'].dropna().unique()]))
                    cpfs_alvo = df_base[df_base['OBRA'].astype(str).isin(obras)]['CPF_LIMPO'].tolist()
                else:
                    txt = st.text_area("Cole os CPFs:")
                    cpfs_alvo = [limpar_cpf(c) for c in re.findall(r'\d+', txt)]
                
                st.metric("Selecionados", len(cpfs_alvo))
                msg = st.text_area("Mensagem:", height=100)
                if st.button("DISPARAR"):
                    df_m = pd.read_csv(FILE_MENSAGENS, dtype=str)
                    novas = []
                    for c in cpfs_alvo:
                        m_colab = df_base[df_base['CPF_LIMPO'] == c]
                        nome = str(m_colab.iloc[0]['Nome']) if not m_colab.empty else "Novo"
                        novas.append({
                            "id": datetime.now().strftime("%Y%m%d%H%M%S") + c[-3:],
                            "data_envio": datetime.now().strftime("%d/%m/%Y %H:%M"),
                            "cpf_destino": c, "nome_destino": nome, "mensagem": msg, "lido": "Não", "data_leitura": ""
                        })
                    pd.concat([df_m, pd.DataFrame(novas)], ignore_index=True).to_csv(FILE_MENSAGENS, index=False)
                    st.success("Mensagens enviadas!")

        with t2:
            st.subheader("Gerenciar Mensagens")
            df_rep = carregar_mensagens()
            if not df_rep.empty:
                df_rep['Excluir'] = False
                # Remove a coluna auxiliar dt_obj para não confundir o usuário
                colunas_view = ['Excluir', 'data_envio', 'nome_destino', 'mensagem', 'lido', 'data_leitura', 'id']
                editado = st.data_editor(df_rep[colunas_view], hide_index=True, use_container_width=True)
                
                if st.button("Remover Selecionados", type="primary"):
                    ids_del = editado[editado['Excluir'] == True]['id'].tolist()
                    df_final = df_rep[~df_rep['id'].isin(ids_del)].drop(columns=['dt_obj', 'Excluir'])
                    df_final.to_csv(FILE_MENSAGENS, index=False)
                    st.success("Excluído com sucesso!")
                    st.rerun()
            else: st.write("Sem histórico.")

        with t3:
            up = st.file_uploader("Novo Excel", type="xlsx")
            if st.button("Substituir Base"):
                if up:
                    with open(FILE_COLABS, "wb") as f: f.write(up.getbuffer())
                    st.cache_data.clear()
                    st.rerun()

    else:
        # VISÃO COLABORADOR
        st.title(f"Olá, {st.session_state.user_nome.split()[0]}!")
        df_m = carregar_mensagens()
        
        # Filtro de 48 horas
        limite = datetime.now() - timedelta(hours=48)
        minhas = df_m[(df_m['cpf_destino'] == st.session_state.user_cpf) & (df_m['dt_obj'] >= limite)].copy()
        
        if not minhas.empty:
            # Se houver algo novo, toca som
            if "Não" in minhas['lido'].values:
                st.warning("🔔 Você tem uma nova mensagem!")
                components.html("<script>window.parent.tocarAlerta();</script>", height=0)
            
            for _, r in minhas.iloc[::-1].iterrows():
                with st.chat_message("assistant"):
                    st.write(f"📅 **{r['data_envio']}**")
                    st.markdown(r['mensagem'])
            
            # Marca como lido
            df_total = pd.read_csv(FILE_MENSAGENS, dtype=str).fillna("")
            idx = df_total[(df_total['cpf_destino'] == st.session_state.user_cpf) & (df_total['lido'] == "Não")].index
            if len(idx) > 0:
                df_total.loc[idx, 'lido'] = "Sim"
                df_total.loc[idx, 'data_leitura'] = datetime.now().strftime("%d/%m/%Y %H:%M")
                df_total.to_csv(FILE_MENSAGENS, index=False)
        else:
            st.info("Nenhuma mensagem nova (avisos expiram em 48h).")
